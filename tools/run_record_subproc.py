...
if __name__ == '__main__':
    ...
    print(dataset)
    print(output_folder)
    os.popen("mkdir -p {0}".format(output_folder))
    ...
    while(True):
        ...
        sleep_est = len_dic[seq] * 20 / 1000
        print("seq: ",seq," start")
        launch_proc = subprocess.Popen(["roslaunch", "lego_loam", "run.launch", "output_folder:="+output_folder, "bag_path:="+dataset+".bag"], shell=False)
        time.sleep(10)
        while(detect_active()):
        ...
        tmp = val.readline()
        if((tmp.split()[0] == str(len_dic[seq])) or (tmp.split()[0] == str(len_dic[seq]-1))):
            exit()
        else:
            fail += 1
            print("\033[1;32mERROR {0} times\033[0m".format(str(fail)))
